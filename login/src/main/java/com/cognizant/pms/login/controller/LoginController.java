...
public class LoginController {
    ...
    public ResponseEntity<ApiResponse> validateToken(@RequestHeader("Authorization") String authToken) throws LoginException{
    	try {
    		return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(),"Token Validated",jwtUtil.validateToken(authToken)));
    	}catch(Exception e) {
    		return ResponseEntity.ok(new ApiResponse(HttpStatus.UNAUTHORIZED.value(),"Invalid Token",jwtUtil.validateToken(authToken)));
    	}
    }
